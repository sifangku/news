...
$(function(){
	$('button.delete-type').click(function(){
		$('#myModal .modal-body').html('<div class="alert alert-warning" role="alert">您真的要删除管理员 <span style="color:red;font-weight:bold;">'+$(this).data('name')+'</span> 吗？</div>');
	...
	$('#myModal .modal-footer button:last-child').click(function(){
		...
		//alert($(this).data('id'));
		$.post('<?php echo Url::getUrl(array(C=>'manage',M=>'delete'))?>',{id:$(this).data('id')},function(responseText){
			if(responseText.status){
				window.location.href='<?php echo Url::getUrl(array(C=>'manage',M=>'index'))?>';
	        }else{
	...
	<div class="row">
		...
		<div class="col-md-10">
			<table class="sfk-table table table-hover">
		      ...
		      <tbody>
		      	<?php 
		      	foreach ($this->getData('manage') as $val){
					$power='';
					foreach (Power::get(MODULE) as $controller){
						foreach ($controller as $method){
							if(Power::has($val['power'],$method['pmark'])){
								$power.=$method['note'].'，';
							}
						}
					}
		        ...
		        <tr>
		          ...
		          <td><span data-toggle="tooltip" data-placement="left" title="<?php echo $power?>"><?php echo $val['power']?></span></td>
		          <td><?php echo date('Y-m-d',$val['rtime'])?></td>
		          <td>
		          	<a href="<?php echo Url::getUrl(array(C=>'manage',M=>'update','id'=>$val['id']))?>" type="button" class="btn btn-primary btn-xs">更改</a>&nbsp;&nbsp;
