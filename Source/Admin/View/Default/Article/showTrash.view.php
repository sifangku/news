...
$(function(){
	$('button.delete-type,button.revert').click(function(){
		if($(this).hasClass('delete-type')){
			var message='您真的要 <span style="color:red;font-weight:bold;">永久删除</span> 新闻 <span style="color:red;font-weight:bold;">'+$(this).data('name')+'</span>';
		}else{
			var message='您确定要还原新闻 <span style="color:red;font-weight:bold;">'+$(this).data('name')+'</span>';
		}
		$('#myModal .modal-body').html('<div class="alert alert-warning" role="alert">'+message+'</div>');
		$('#myModal .modal-footer button:last-child').data('id',$(this).data('id'));
		$('#myModal .modal-footer button:last-child').data('url',$(this).data('url'));
		$('#myModal').modal();
	});
	$('#myModal .modal-footer button:last-child').click(function(){
		$('#myModal .modal-body').html('<p>处理中...</p>');
		//alert($(this).data('id'));
		$.post($(this).data('url'),{id:$(this).data('id')},function(responseText){
			if(responseText.status){
				window.location.href='<?php echo Url::getUrl(array(C=>'article',M=>'showTrash'))?>';
	        }else{
	...
	<div class="row">
		...
		<div class="col-md-10">
			<table class="sfk-table table table-hover">
		      ...
		      <tbody>
		      	<?php 
		      	foreach ($this->getData('article') as $val){
		      	?>
		        <tr>
		          <td><input style="width:50px;height:26px;" class="form-control input-sm" type="text" value="<?php echo $val['weights']?>"></td>
		          <td><?php echo $val['id']?></td>
		          <th><?php echo $val['title']?></th>
		          <th><?php echo $val['name']?></th>
		          <th><?php echo $val['editor']?></th>
		          <th><?php echo date('Y-m-d',$val['pubdate'])?></th>
		          <td>
		          	<button data-id="<?php echo $val['id']?>" data-name="<?php echo $val['title']?>" data-url="<?php echo Url::getUrl(array(C=>'article',M=>'revert'))?>" type="button" class="revert btn btn-primary btn-xs">还原</button>&nbsp;&nbsp;
		          	<button data-id="<?php echo $val['id']?>" data-name="<?php echo $val['title']?>" data-url="<?php echo Url::getUrl(array(C=>'article',M=>'trueDelete'))?>" type="button" class="delete-type btn btn-primary btn-xs">永久删除</button>
		          </td>
		        </tr>
		        <?php
		        }
