...
$(function(){
	$('button.delete-type').click(function(){
		$('#myModal .modal-body').html('<div class="alert alert-warning" role="alert">您真的要删除新闻 <span style="color:red;font-weight:bold;">'+$(this).data('name')+'</span></div>');
	...
	$('#myModal .modal-footer button:last-child').click(function(){
		...
		//alert($(this).data('id'));
		$.post('<?php echo Url::getUrl(array(C=>'article',M=>'delete'))?>',{id:$(this).data('id')},function(responseText){
			if(responseText.status){
				window.location.href='<?php echo Url::getUrl(array(C=>'article',M=>'index'))?>';
	        }else{
			...
			<table class="sfk-table table table-hover">
		      ...
		      <tbody>
		        ...
		        <tr>
		          ...
		          <td>
		          	<a href="<?php echo Url::getUrl(array(E=>'index.php',C=>'index',M=>'article','uname'=>$val['uname'],'pubdate'=>$val['pubdate'],'id'=>$val['id']))?>" target="_blank" type="button" class="btn btn-primary btn-xs">预览</a>&nbsp;&nbsp;
		          	<a href="<?php echo Url::getUrl(array(C=>'article',M=>'update','id'=>$val['id']))?>" type="button" class="btn btn-primary btn-xs">更改</a>&nbsp;&nbsp;
		          	<button data-id="<?php echo $val['id']?>" data-name="<?php echo $val['title']?>" type="button" class="delete-type btn btn-primary btn-xs">删除</button>
		          </td>
		        </tr>
		        <?php
		        }
